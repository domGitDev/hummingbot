...
from hummingbot.connector.exchange.dexfin.dexfin_in_flight_order import DexfinInFlightOrder
from hummingbot.connector.exchange.dexfin.dexfin_order_book_tracker import DexfinOrderBookTracker
from hummingbot.connector.exchange.dexfin.dexfin_user_stream_tracker import DexfinUserStreamTracker
...
DEXFIN_ROOT_API = "https://test.dexfin.dev"


class DexfinAPIError(IOError):
    def __init__(self, error_payload: Dict[str, Any]):
        super().__init__()
        self.error_payload = error_payload


cdef class DexfinExchangeTransactionTracker(TransactionTracker):
    cdef:
        DexfinExchange _owner

    def __init__(self, owner: DexfinExchange):
        super().__init__()
        self._owner = owner
...
cdef class DexfinExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._dexfin_auth = DexfinAuth(api_key=dexfin_api_key, secret_key=dexfin_secret_key)
        self._in_flight_orders = {}
        self._last_poll_timestamp = 0
        self._last_timestamp = 0
        self._order_book_tracker = DexfinOrderBookTracker(trading_pairs)
        ...
        self._tx_tracker = DexfinExchangeTransactionTracker(self)
        self._user_stream_tracker = DexfinUserStreamTracker(dexfin_auth=self._dexfin_auth)
    ...
    @property
    def order_book_tracker(self) -> DexfinOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, DexfinInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: DexfinInFlightOrder.from_json(value)
    ...
    @property
    def user_stream_tracker(self) -> DexfinUserStreamTracker:
        return self._user_stream_tracker

    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await DexfinAPIOrderBookDataSource.get_active_exchange_markets()
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if execution_status != "done":
                    if Decimal(execution_data["executed_volume"]) > 0:
                        execute_amount_diff = Decimal(execution_data["executed_volume"])
                        execute_price = Decimal(execution_data["price"])
                        tracked_order.executed_amount_base = Decimal(execution_data["executed_volume"])
                        tracked_order.executed_amount_quote = Decimal(execution_data["executed_volume"]) * Decimal(
                            execute_price)
    ...
    async def _api_request(self,
        ...
        url = DEXFIN_ROOT_API + path_url
        client = await self._http_client()
        if is_auth_required:
            if is_partner_required:
                headers = self._dexfin_auth.add_auth_to_params(method, path_url, params, partner_header=True)
            else:
                headers = self._dexfin_auth.add_auth_to_params(method, path_url, params)
        else:
    ...
    async def _update_balances(self):
        cdef:
            str path_url = "/api/v2/peatio/account/balances"
            str asset_name
        ...
        if data:
            for balance_entry in data:
                ...
                self._account_available_balances[asset_name] = Decimal(balance_entry["balance"]) - locked
    ...
    cdef object c_get_fee(self,
        ...
        # Fee info from https://www.dexfin.com/vip/fee
        """
        if order_type is OrderType.LIMIT and fee_overrides_config_map["dexfin_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["dexfin_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["dexfin_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["dexfin_taker_fee"].value / Decimal("100"))
        return TradeFee(percent=Decimal("0.001"))
        """
        is_maker = order_type is OrderType.LIMIT_MAKER
        return estimate_fee("dexfin", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self._api_request("get", path_url="/api/v2/peatio/public/markets")
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                ...
                maker_fee = Decimal(order_update.get("maker_fee", 0))
                taker_fee = Decimal(order_update.get("taker_fee", 0))
                tracked_order.executed_amount_base = Decimal(order_update["executed_volume"])
                tracked_order.executed_amount_quote = Decimal(order_update["executed_volume"]) * execute_price
                tracked_order.fee_paid = maker_fee + taker_fee
                order_filled_event = OrderFilledEvent(
                    ...
                    tracked_order.order_type,
                    float(execute_price),
                    float(Decimal(order_update["remaining_volume"])),
                    self.c_get_fee(
                        tracked_order.base_asset,
                        tracked_order.quote_asset,
                        tracked_order.order_type,
                ...
                execute_amount_diff = Decimal(order_update["executed_volume"])
                self.logger().info(f"Filled {execute_amount_diff} out of {tracked_order.amount} of the order {tracked_order.client_order_id}.")
                self.c_trigger_event(self.MARKET_ORDER_FILLED_EVENT_TAG, order_filled_event)

                if order_state in "done":
                    self.c_stop_tracking_order(tracked_order.client_order_id)
    ...
    async def place_order(self,
        ...
        return str(exchange_order_id["tid"])
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Dexfin for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Dexfin. Check API key and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = order_type.name.lower()
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Dexfin for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Dexfin. Check API key and network connection."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Dexfin. "
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel all orders on Dexfin. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object price,
                                object amount):
        self._in_flight_orders[client_order_id] = DexfinInFlightOrder(
