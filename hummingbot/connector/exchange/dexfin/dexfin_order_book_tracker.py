...
class DexfinOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
        ...
        super().__init__(DexfinAPIOrderBookDataSource(trading_pairs), trading_pairs)
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._saved_message_queues: Dict[str, Deque[DexfinOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, DexfinActiveOrderTracker] = defaultdict(DexfinActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: DexfinOrderBookMessage = await self._order_book_diff_stream.get()
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[DexfinOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: OrderBook = self._order_books[trading_pair]
        active_order_tracker: DexfinActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: DexfinOrderBookMessage = None
                saved_messages: Deque[DexfinOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[DexfinOrderBookMessage] = list(past_diffs_window)
