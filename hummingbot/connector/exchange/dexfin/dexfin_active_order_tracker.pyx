...
DexfinOrderBookTrackingDictionary = Dict[Decimal, Dict[str, Dict[str, any]]]

cdef class DexfinActiveOrderTracker:
    def __init__(self,
                 active_asks: DexfinOrderBookTrackingDictionary = None,
                 active_bids: DexfinOrderBookTrackingDictionary = None):
        super().__init__()
    ...
    @property
    def active_asks(self) -> DexfinOrderBookTrackingDictionary:
        return self._active_asks

    @property
    def active_bids(self) -> DexfinOrderBookTrackingDictionary:
    ...
    cdef tuple c_convert_diff_message_to_np_arrays(self, object message):
        cdef:
            ...
            list bid_entries = content.get("bids", 0)
            list ask_entries = content.get("asks", 0)
            str order_id
            str order_side
            str price_raw
            object price
    ...
    cdef tuple c_convert_snapshot_message_to_np_arrays(self, object message):
        ...
        for snapshot_orders, active_orders in [(message.content.get("bids", 0), self._active_bids), (message.content.get("asks", 0), self.active_asks)]:
    ...
    cdef np.ndarray[np.float64_t, ndim=1] c_convert_trade_message_to_np_array(self, object message):
        ...
        return np.array(
            [message.timestamp, trade_type_value, float(message.content["price"]), float(message.content["amount"])],
            dtype="float64"
        )
